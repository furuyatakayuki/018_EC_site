...
// キーワードから商品情報を取得
function query_get($query, $sort, $category_id){
    if ($query != "") {
        global $appid;
...
// itemcodeから商品情報を取得
// 存在しないitemcodeの場合もnullを返す
function itemcode_get($itemcode){
    if ($itemcode != "") {
        global $appid;
        // 情報量はresponsegroupで設定
        $volume = "medium";
        // image_sizeは300か600で大きな画像を取得
        $url = "https://shopping.yahooapis.jp/ShoppingWebService/V1/itemLookup?appid=$appid&responsegroup=$volume&itemcode=$itemcode&image_size=300";
        $xml = simplexml_load_file($url);
        if ($xml["totalResultsReturned"] != 0) {
            return $xml->Result->Hit;
        }
